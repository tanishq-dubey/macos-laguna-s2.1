...
def test_chart_sources_have_expected_laguna_results():
    ...
    q3xl = next(item for item in local if item.label == "Q3XL")
    ...
    assert q3xl.decode_tps == pytest.approx(49.14, abs=0.005)
    assert q3xl.peak_memory_gb == pytest.approx(51.41, abs=0.005)
    assert q3xl.suite_score == pytest.approx(0.812, abs=0.001)
