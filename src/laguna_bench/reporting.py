...
def build_comparison(output_root: Path) -> tuple[Path, list[dict[str, Any]]]:
        ...
        rows.append(
            {
                ...
                "peak_memory_gb": max((metrics.get("peak_memory_gb", 0.0) for metrics in all_metrics), default=0.0),
    ...
    lines = [
        "# Laguna S 2.1 MLX comparison",
        "",
        "Latest complete six-task run per model.",
        "",
        "| Model | Engine | Score | Gen | Agent | Gen tok/s | Peak GB | Load s |",
        "|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            f"| {row['model']} | {row['engine']} | {row['score']:.3f} | {row['generation_score']:.3f} | "
            f"{row['agentic_score']:.3f} | {row['generation_tps']:.2f} | {row['peak_memory_gb']:.2f} | {row['load_seconds']:.2f} |"
        )
